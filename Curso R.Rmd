...
9. Crie um objeto com nome "r" contendo os números 1,2 e 3 repetidos 100 vezes (ex. 1,2,3,1,2,3...).**DICA**: use a função _"rep"_ para resolver este exercício.  
...
Use a função _"class"_ para descobrir o tipo de objeto (númerico, caractere, lógico)
```{r}
class(v1)
```


A funcao "str" e similar a função "class" e indica o tipo, o tamanho, e os elementos de um objeto
...
Algumas operações matematicas usando vetores salvos como objetos
```{r}
v30 <- c(1, 2, 3, 4)
v31 <- 1
...
Vetores categóricos ou fatores são um tipo especial de vetores de caracteres. Alguns exemplos de categorias. Desempenho dos alunos no curso do R (bom, regular, baixo); horas de estudo (1,4,8, >12); anafalbetismo (crianças, jovens, adultos)
```{r}
...
#quantos valores são iguais a 1?
```{r}
